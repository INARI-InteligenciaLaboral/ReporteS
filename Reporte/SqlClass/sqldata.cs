...
namespace Reporte.SqlClass
{
    public class sqldata
    {
        ...
        {
            string m_cadena = "Persist Security Info=False;User ID=sa;Password=Hardstyle1292;Initial Catalog=Siap_pruebas;Data Source=HANCOCK\\HANCOCKBOA";
        ...
        public static DataTable ObtenerPro(string m_empresa)
        {
            ...
            try
            {
                ...
                {
                    m_conexion.Open();
                    string m_command = "SELECT pro_keypro AS KeyPro, pro_despro AS DesPro FROM nmloproc WHERE pro_keycia = " + m_empresa;
                    SqlCommand m_adapter = new SqlCommand(m_command, m_conexion);
        ...
        public static DataTable GenerarReporte(CascadingDropdownsModel m_Parametros)
        {
            ...
            try
            {
                using (SqlConnection m_conexion = new SqlConnection(m_cadena))
                {
                    ...
                    cmd.Parameters.Add("@m_procesos", SqlDbType.Int, 10).Value = m_Parametros.SelectedProcesos;
